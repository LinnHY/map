...
class MMIMDBDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.missing_table[image_index] == 2 or simulate_missing_type == 2:
            ...
            sentence="This image shows "+caption+".According to'"+text+"',the genre of this film can be classified as [MASK]"
            ...
            if self.missing_table[image_index] == 1 or simulate_missing_type == 1:
                text = ''
                caption=str(self.table['caption'][image_index])
                sentence="This image shows "+caption+",it can be classified as [MASK]"
                encoding = self.tokenizer(
                    sentence,
                    padding="max_length",
                    truncation=True,
        ...
        #print("self.table[image_id]=",self.table["image_id"][image_index])
        id=self.table["image_id"][image_index].as_py()
